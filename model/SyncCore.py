...
def init(count, url, methods='GET', params='{}', read=False):
    ...
    if read:
        ...
        for requestId in range(count):
            request = sync.SyncRequestTask(requestId, url, methods, params, headers)
            syncPool.append(request)
    log.logger.info('初始化消耗时间 {:.2f} 毫秒'.format((time.time() - start_time) * 1000))
...
def fast_run(begin, end):
    ...
    for request in syncPool[int(begin):int(end)]:
        request.start()
    log.logger.warning("请求消耗时间 {:.2f} 毫秒".format((time.time() - run_start_time) * 1000))
...
def slow_run(begin, end, slowTime):
    ...
    if slowTime == 0:
        ...
        waitTime = round(slowTime / len(syncPool), 3)
        log.logger.info("延迟时间为{}毫秒".format(waitTime * 1000))
        for request in syncPool[int(begin):int(end)]:
            request.start()
            time.sleep(waitTime)
        log.logger.warning("请求消耗时间 {:.2f}".format((time.time() - run_start_time) * 1000))
...
def out(length):
    ...
    # 打印成功请求的request中信息
    for request in sync.success:
        log.logger.debug(request.content)
    # 打印失败请求的request中信息
    for request in sync.fail:
...
def start(slowTime, roundCount, thread_count, request_url, method, param, read=False):
    ...
    init(thread_count * roundCount, request_url, method, param, read)
    if slowTime == 0:
